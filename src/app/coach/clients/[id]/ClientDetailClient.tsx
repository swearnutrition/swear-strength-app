...
interface Stats {
  ...
  completedInPersonSessions: number
  upcomingInPersonSessions: number
  soloWorkouts: number
}
...
export function ClientDetailClient({
  ...
  // Session notes state
  const [editingSessionNotes, setEditingSessionNotes] = useState<string | null>(null)
  ...
  const handleUpdateClientType = async (newType: ClientType, sessions?: number) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          client_type: newType,
          ...(newType === 'hybrid' && sessions ? { hybrid_sessions_per_month: sessions } : {}),
        })
        .eq('id', client.id)
      ...
      if (sessions) setHybridSessions(sessions)
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h3 className="text-sm font-medium text-slate-900 dark:text-white">Client Type</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {clientType === 'online' && 'Online clients can book monthly check-ins only'}
                {clientType === 'training' && 'Training clients can book sessions and check-ins'}
                {clientType === 'hybrid' && `Hybrid clients can book up to ${hybridSessions} sessions/month + check-ins`}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700">
                {(['online', 'training', 'hybrid'] as const).map((type) => (
                  <button
                    key={type}
                    onClick={() => handleUpdateClientType(type, type === 'hybrid' ? hybridSessions : undefined)}
              ...
              {clientType === 'hybrid' && (
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-500 dark:text-slate-400">Sessions/mo:</span>
                  <select
                    value={hybridSessions}
                    onChange={(e) => handleUpdateClientType('hybrid', parseInt(e.target.value))}
                    disabled={savingClientType}
                    className="px-2 py-1.5 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
                  >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">In-Person Sessions</p>
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.completedInPersonSessions}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">completed with coach</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">Solo Workouts</p>
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.soloWorkouts}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">logged in app</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">Upcoming</p>
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.upcomingInPersonSessions}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">sessions booked</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'history' && (
          <div className="space-y-6">
            {/* Combined Workout History */}
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className="p-5 border-b border-slate-200 dark:border-slate-800">
                <h2 className="font-semibold text-slate-900 dark:text-white">Complete Workout History</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">In-person sessions and solo workouts</p>
              </div>

              <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {/* Merge and sort bookedSessions and workoutLogs by date */}
                {(() => {
                  // Create combined list with type discrimination
                  const combined: Array<
                    | { type: 'booking'; data: BookedSession }
                    | { type: 'workout'; data: WorkoutLog }
                  > = [
                    ...bookedSessions
                      .filter(s => s.bookingType === 'session')
                      .map(s => ({ type: 'booking' as const, data: s })),
                    ...workoutLogs.map(w => ({ type: 'workout' as const, data: w })),
                  ]

                  // Sort by date descending
                  combined.sort((a, b) => {
                    const dateA = new Date(a.type === 'booking' ? a.data.startsAt : a.data.date)
                    const dateB = new Date(b.type === 'booking' ? b.data.startsAt : b.data.date)
                  ...
                  if (combined.length === 0) {
                    return (
                      <div className="p-8 text-center">
                        <p className="text-slate-400 dark:text-slate-500">No workout history yet</p>
                      </div>
                    )
                  }
                  ...
                  return combined.map((item) => {
                    if (item.type === 'booking') {
                      ...
                      const statusColors: Record<string, string> = {
                        ...
                        confirmed: 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400',
                        cancelled: 'bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400',
                        no_show: 'bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400',
                      }

                      return (
                        <div key={`booking-${session.id}`} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="font-medium text-slate-900 dark:text-white">In-Person Training Session</span>
                              ...
                              <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">
                                {sessionDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })} - {endDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                              </p>

                              {/* Notes Section */}
                              {session.status === 'completed' && (
                                <div className="mt-3">
                                  {editingSessionNotes === session.id ? (
                                    <div className="space-y-2">
                                      <textarea
                                        value={sessionNotes}
                                        onChange={(e) => setSessionNotes(e.target.value)}
                                        placeholder="Add notes about this session..."
                                        className="w-full p-3 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
                                        rows={3}
                                      />
                                      <div className="flex gap-2">
                                        <button
                                          onClick={async () => {
                                            setSavingNotes(true)
                                            try {
                                              const res = await fetch(`/api/bookings/${session.id}/notes`, {
                                                method: 'PATCH',
                                                headers: { 'Content-Type': 'application/json' },
                                                body: JSON.stringify({ notes: sessionNotes }),
                                              })
                                              if (!res.ok) throw new Error('Failed to save notes')
                                              // Update local state - would need to refresh or update bookedSessions
                                              window.location.reload()
                                            } catch (err) {
                                          ...
                                          className="px-3 py-1.5 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50"
                                        >
                                        ...
                                        <button
                                          ...
                                          className="px-3 py-1.5 text-slate-600 dark:text-slate-400 text-sm font-medium hover:text-slate-900 dark:hover:text-white transition-colors"
                                        >
                                          Cancel
                                        </button>
                                      </div>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">
                                      <div className="flex items-start justify-between gap-2">
                                        <div>
                                          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Coach Notes</p>
                                          <p className="text-sm text-slate-700 dark:text-slate-300">{session.notes}</p>
                                        </div>
                                        <button
                                          onClick={() => {
                                            setEditingSessionNotes(session.id)
                      ...
                      // Workout log (solo workout)
                      const log = item.data
                      const workoutDate = new Date(log.date)

                      return (
                        <div key={`workout-${log.id}`} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                          <div className="flex items-start gap-4">
                            <div className="w-14 text-center flex-shrink-0">
                              <p className="text-xs text-slate-400 dark:text-slate-500 uppercase">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="font-medium text-slate-900 dark:text-white">{log.dayName}</span>
                                {!log.completed && (
                                ...
                                {log.section && log.completed && (
                                  <span className="px-2 py-0.5 bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 rounded text-xs font-medium">
                                    {log.section}
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                                <span>{log.setCount} sets</span>
                                <span>{log.totalVolume >= 1000 ? `${(log.totalVolume / 1000).toFixed(1)}k` : log.totalVolume} lbs</span>
                                {log.difficulty && (
                              ...
                              {log.notes && (
                                <div className="mt-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">
                                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Client Notes</p>
