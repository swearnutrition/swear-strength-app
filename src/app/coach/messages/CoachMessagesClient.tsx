...
import { useScheduledMessages, useMassDM } from '@/hooks/useScheduledMessages'
...
type TabType = 'clients' | 'groups' | 'scheduled'
...
export function CoachMessagesClient({ userId, userName }: CoachMessagesClientProps) {
  ...
  // Filter out pending clients - they can't receive messages until they sign up
  const activeClients = clients.filter((client) => !client.isPending)

  // Get active clients who don't have a conversation yet
  const clientsWithoutConversation = activeClients.filter(
  ...
  const handleBroadcastSend = async (recipientIds: string[], content: string, scheduledFor?: string) => {
    if (scheduledFor) {
      // Schedule mass DM
      ...
      if (error) {
        alert(`Failed to schedule broadcast: ${error}`)
      } else {
        await refreshScheduled()
      }
    } else {
      // Send immediately
      const result = await sendMassDM(recipientIds, content)
      if (!result.success) {
        alert(`Failed to send to ${result.failed} recipients`)
      }
      await refetch() // Refresh conversations to show new messages
    }
  }
    ...
    <div className="h-[calc(100vh-4rem)] flex">
      ...
      <div className="w-80 border-r border-slate-800 flex flex-col bg-slate-900/50">
        ...
        {activeTab === 'clients' ? (
          ...
          <div className="flex-1 overflow-y-auto">
            {scheduledLoading ? (
              <div className="p-4 text-center text-slate-500">Loading scheduled messages...</div>
            ) : scheduledMessages.length === 0 ? (
              <div className="p-4 text-center text-slate-500">
                <svg className="w-12 h-12 mx-auto mb-3 text-slate-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p className="text-xs text-slate-600 mt-1">Schedule messages using the clock icon when composing</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-800">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Content preview */}
                        <p className="text-white text-sm line-clamp-2 mb-2">{msg.content}</p>
                        {/* Scheduled time */}
                        <div className="flex items-center gap-1 text-xs text-slate-500">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          {new Date(msg.scheduledFor).toLocaleString(undefined, {
                            weekday: 'short',
                            month: 'short',
                            day: 'numeric',
                            hour: 'numeric',
        ...
        <div className="space-y-2">
          {loadingClients ? (
            ...
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-xs mt-1">Clients will appear here once they sign up</p>
            </div>
      ...
      {/* Schedule Modal for individual DMs */}
      <ScheduleModal
